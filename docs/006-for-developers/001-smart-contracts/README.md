...
The PCOArt system allows the Artist to defer the minting of tokens in their collection until the end of their first [Stewardship Inauguration](../stewardship-inauguration). All of the art and metadata are still defined during the creation process, but the on-chain representation of the asset won't be created initially.&#x20;
...
- The artist could set the first on-chain Stewardship Inauguration to the equivalent of 1 [Stewardship Cycle](../pco-settings) into the future, run an offline auction, and manually transfer the token to the winner.
